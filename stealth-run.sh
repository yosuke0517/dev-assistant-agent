...
STEP5: レビュー結果を以下の形式でまとめてください：

---
# PRレビュー結果: Issue #${ISSUE_ID}
...
【クロスリポジトリ対応 - 必須承認フロー】
以下の関連リポジトリにもアクセス可能です。
${CROSS_REPO_PROMPT}

【重要】実装を開始する前に、必ず以下の承認フローを実行してください：
1. 課題内容を確認し、各リポジトリで何を行うか計画を立ててください
2. ask_human MCPツールを使用して、以下の形式でユーザーに確認してください：
   「以下の計画で実装を進めてよろしいですか？
   - [プライマリリポジトリ名]: [実装内容の概要]
   - [関連リポジトリ名]: [実装内容の概要]
   承認いただけましたら実装を開始します。」
3. ユーザーの承認を得てから実装を開始してください
4. ユーザーから修正指示があった場合は計画を修正し、再度確認してください
※ 承認なしに実装を開始することは禁止です。
