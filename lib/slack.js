...
export async function waitForSlackReply(channel, threadTs, afterTs, options = {}) {
    ...
    while (Date.now() < deadline) {
        try {
            ...
            if (data.ok && data.messages) {
                ...
                console.error('Slack conversations.replies APIエラー:', data.error || 'unknown', '(channel:', channel, 'ts:', threadTs, ')');
                apiErrorLogged = true;
            }
        } catch (err) {
    ...
    if (apiErrorLogged) {
        console.log('Slack返信待ちタイムアウト（※ APIエラーが発生していたため、返信を取得できなかった可能性があります）');
    } else {
