    ...
    if (!channel || !threadTs) {
        return {
            content: [
                {
                    type: 'text',
                    text: 'Slack未設定エラー: SLACK_CHANNEL または SLACK_THREAD_TS が設定されていないため、ユーザーへの質問ができません。',
                },
            ],
            isError: true,
        };
    ...
    if (!questionTs) {
        return {
            content: [
                {
                    type: 'text',
                    text: 'Slack送信失敗: 質問の送信に失敗しました（投稿結果が取得できませんでした）。',
                },
            ],
            isError: true,
        };
    ...
    if (!reply) {
        return {
            content: [
                {
                    type: 'text',
                    text: `タイムアウト: 質問はSlackに送信済みですが、${Math.floor(timeoutMs / 60_000)}分以内に返信がありませんでした。`,
                },
            ],
        };
    }
