...
STEP1: 以下のghコマンドで ${GITHUB_REPO} リポジトリの Issue #${ISSUE_ID} の内容を取得してください。これが元の仕様です。
\`\`\`
...
STEP3: 見つけたPRのレビューコメントを全量取得してください。以下のghコマンドを使用してください：
\`\`\`
gh api repos/${GITHUB_REPO}/pulls/<PR番号>/reviews --paginate
gh api repos/${GITHUB_REPO}/pulls/<PR番号>/comments --paginate
gh pr view <PR番号> --repo ${GITHUB_REPO} --json comments
\`\`\`
※ GitHub MCPの get_pull_request_reviews / get_pull_request_comments ではなく、上記のghコマンドを使用してください。ghコマンドの方が --paginate オプションにより全量取得できます。
...
STEP7: PRに対応完了のコメントを投稿してください。以下のghコマンドを使用してコメントしてください：
\`\`\`
gh pr comment <PR番号> --repo ${GITHUB_REPO} --body \"<コメント内容>\"
\`\`\`
以下の形式でコメントしてください：
---
## レビューFB対応完了
